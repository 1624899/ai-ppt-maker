...
from ppt_system.background_removal import (
    BACKGROUND_REMOVAL_STRATEGY_BUILTIN_ALPHA_REFINE,
...
class BackgroundRemovalTests(unittest.TestCase):
    ...
    def test_make_transparent_returns_builtin_alpha_refine_result(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result.strategy, BACKGROUND_REMOVAL_STRATEGY_BUILTIN_ALPHA_REFINE)
            self.assertIsNone(result.warning)

    def test_builtin_alpha_refine_removes_inner_white_region_when_using_global_white_knockout(self) -> None:
    ...
    def test_builtin_alpha_refine_preserves_soft_edge_alpha_for_border_connected_background(self) -> None:
        image = Image.new("RGBA", (3, 1), (255, 255, 255, 255))
        image.putpixel((0, 0), (255, 255, 255, 255))
        image.putpixel((1, 0), (245, 245, 255, 255))
        image.putpixel((2, 0), (0, 90, 220, 255))
        ...
        self.assertEqual(result.image.getpixel((2, 0))[3], 255)

    def test_builtin_alpha_refine_removes_disconnected_weak_speck(self) -> None:
        image = Image.new("RGBA", (12, 12), (255, 255, 255, 255))
        pixels = image.load()
        for x in range(2, 8):
            pixels[x, 6] = (30, 80, 220, 255)
    ...
    def test_builtin_alpha_refine_preserves_soft_edge_when_attached_to_real_foreground(self) -> None:
        image = Image.new("RGBA", (7, 7), (255, 255, 255, 255))
        pixels = image.load()
        for y in range(2, 5):
            for x in range(2, 5):
        ...
        self.assertEqual(result.strategy, BACKGROUND_REMOVAL_STRATEGY_BUILTIN_ALPHA_REFINE)
        self.assertGreater(result.image.getpixel((1, 3))[3], 0)

...
const MobileApp = () => {
  ...
  const savePlan = async () => { if (!selectedJob || !plan) return; setBusy('save'); try { await api(`/api/jobs/${selectedJob.job_id}/plan`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(plan) }); setError('规划已保存'); await openJob(selectedJob.job_id); } catch (reason) { setError(reason.message); } finally { setBusy(''); } };
  const confirmPlan = async () => { if (!selectedJob || !plan) return; setBusy('confirm-plan'); try { await api(`/api/jobs/${selectedJob.job_id}/plan/confirm`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ plan }) }); await openJob(selectedJob.job_id); } catch (reason) { setError(reason.message); } finally { setBusy(''); } };
  const confirmReferences = async () => { if (!selectedJob) return; setBusy('confirm-references'); try { await api(`/api/jobs/${selectedJob.job_id}/reference/confirm`, { method: 'POST' }); await openJob(selectedJob.job_id); } catch (reason) { setError(reason.message); } finally { setBusy(''); } };

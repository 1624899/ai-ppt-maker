...
## ⚠️ 注意
本人用的模型为 **gpt-5.6 sol+ gpt-image-2/2.5**，没尝试过其他模型的效果，使用其他模型的效果暂时请自己探索。如若你用的是**中转站api**，请**务必把代理关闭**，不然图片可能无法正常生成
...
仓库中的 `config.json` 是可提交的默认模板；本机私有覆盖写在 `config.local.json`，该文件已加入 `.gitignore`，不会提交。运行时会先读取 `config.json`，再合并同目录的 `config.local.json`，Web 设置页保存时也会优先写入本地覆盖文件。
...
系统支持配置多组对话模型和生图模型，通过 Web 界面可视化管理。模型端点和激活模型可以保存在 `config.local.json`，API Key 不写入配置 JSON，统一通过环境变量安全存储，首次在 Web 界面保存时会自动创建 `.env` 文件。

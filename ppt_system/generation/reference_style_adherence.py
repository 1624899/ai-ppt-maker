...
_REFERENCE_STYLE_ADHERENCE_PROMPTS: dict[str, tuple[str, str]] = {
    "loose": (
        "优先学习原稿图的版芯比例、留白、背景纹理、线条样式、卡片层级和色彩节奏，再围绕本页内容重新设计。",
        "不要照搬某一张原稿图的具体版式，只需要保持同系列视觉一致性。",
    ),
    "balanced": (
        "优先解析原稿图的版芯比例、留白、背景纹理、线条样式、卡片层级与色彩节奏，将其作为设计的基准约束。",
        "保留适度发挥空间，做到“框架统一、细节鲜活”，避免刻板套用。",
    ),
    "strict": (
        "严格锁定原稿图的版芯比例、留白、背景纹理、线条样式、卡片层级和色彩节奏，然后按照这些规则将本页内容填入。",
...
_REFERENCE_STYLE_PLANNING_GUIDANCE: dict[str, str] = {
    ...
    "balanced": "把原稿图当成统一框架，在不偏离核心秩序的前提下允许适度变化。",
    "strict": "把原稿图当成强约束模板，优先锁定骨架、层级与色彩节奏，再填入本页内容。",
}
...
def build_reference_style_adherence_planning_guidance(
    ...
    *,
    ...
    if has_reference_images:
        return f"原稿图约束强度：{label}。{guidance}"
    return f"原稿图约束强度：{label}。当前未上传原稿图，后续绑定原稿图后按该强度生效。"

...
源码运行时默认使用项目目录下的 `.env`、`config.local.json` 和 `output/`。打包为 Windows exe 后，默认改用 `%APPDATA%\AI PPT Maker\` 保存私有配置、密钥、任务数据库和产物；可通过 `PPT_SYSTEM_DATA_DIR` 或 `PPT_SYSTEM_DATA_MODE=portable` 覆盖。详见 [Windows 桌面打包说明](desktop-packaging.md)。

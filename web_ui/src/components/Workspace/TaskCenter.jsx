...
import { getStyleReferenceImages } from '../../utils/jobPresentation';import { uiClassName } from "../../utils/uiClassName";
...
const TaskCenter = ({
  ...
  const renderTaskCard = (job) => {
    ...
    return (
      <StaggerItem
        ...
        <button
          type="button"
          className={uiClassName("task-card__main")}
          onClick={() => onSelectJob(job.job_id)}
          aria-label={`打开任务：${job.title || '未命名任务'}`}>
          
          <SlideImage
            className={uiClassName("task-card__preview")}
            src={job.preview_image}
            alt={job.title || '任务预览'}
            variant="mini"
            emptyTitle="P" />
          
          <span className={uiClassName("task-card__content")}>
            {renaming ?
            <input
              className={uiClassName("task-card__rename-input")}
              value={renameValue}
              autoFocus
              disabled={pending}
        ...
        {!renaming &&
        <div className={uiClassName("task-card__menu-wrap")}>
            <button
            type="button"
            className={uiClassName("task-card__menu-button")}
            aria-label="任务操作"
            ...
            <button type="button" role="menuitem" onClick={() => pinJob(job)} disabled={pending}>
              ...
              type="button"
              role="menuitem"
              className={uiClassName("is-danger")}
  ...
  return (
    <aside className={uiClassName("workspace-panel task-center")}>
      <div className={uiClassName("workspace-panel__header")}>
        <span className={uiClassName("eyebrow")}>任务中心</span>
        <h2>围绕一份 PPT 持续协作</h2>
        <p>历史任务、项目素材和模板入口都放在这里</p>
      </div>

      <div className={uiClassName("task-center__body")}>
        <ScaleButton className={uiClassName("new-task-button")} onClick={onCreateTask}>
          <PlusCircle size={18} />
          <span>新建 PPT 任务</span>
        </ScaleButton>

        {!loading && pinnedJobs.length > 0 &&
        <section className={uiClassName("task-section task-section--pinned")}>
            <div className={uiClassName("section-title")}>
              <Pin size={15} />
              <span>置顶</span>
            </div>
            <StaggerContainer className={uiClassName("task-list")} itemCount={pinnedJobs.length}>
              {pinnedJobs.map(renderTaskCard)}
            </StaggerContainer>
          </section>
        }

        <section className={uiClassName("task-section")}>
          <div className={uiClassName("section-title")}>
            <Archive size={15} />
            <span>历史任务</span>
          </div>
          <StaggerContainer className={uiClassName("task-list task-list--history")} itemCount={historyJobs.length}>
            {loading ?
            <TaskSkeletonList /> :
            jobs.length === 0 ?
            <div className={uiClassName("empty-state")}>还没有任务，先从中间创建一份初稿。</div> :
            historyJobs.length === 0 ?
            <div className={uiClassName("empty-state")}>历史任务都已置顶。</div> :

            historyJobs.map(renderTaskCard)
            }
          </StaggerContainer>
          {message && <div className={uiClassName("task-action-message")}>{message}</div>}
        </section>

        <section className={uiClassName("task-section task-section--resources")}>
          <div className={uiClassName("section-title")}>
            <FolderKanban size={15} />
            <span>素材与知识库</span>
          </div>
          <StaggerContainer className={uiClassName("resource-list")} itemCount={RESOURCE_LINKS.length}>
            {RESOURCE_LINKS.map(({ key, label, description, icon: Icon, disabled }) =>
            <StaggerItem key={key}>
                <ScaleButton className={uiClassName('resource-link', disabled && 'is-disabled')} disabled={disabled}>
                  <Icon size={17} />
                  <span>
                    <strong>{label}</strong>

...
import {
  ...
  getStageLabel,
  getStatusLabel } from
'../../utils/jobPresentation';import { uiClassName } from "../../utils/uiClassName";
...
const StageIcon = ({ status }) => {
  ...
  if (status === 'running' || status === 'stopping') return <Loader2 size={15} className={uiClassName("spin")} />;
...
const StageProgress = ({ job, dense = false }) => {
  ...
  return (
    <div className={uiClassName('stage-progress', dense && 'stage-progress--dense')}>
      <button type="button" className={uiClassName("stage-progress__summary")} onClick={() => setExpanded((value) => !value)}>
        <span>
          生成进度
          <strong>{status}</strong>
        </span>
        <span className={uiClassName("stage-progress__count")}>{completedCount}/{stages.length}</span>
        <ChevronDown className={uiClassName('stage-progress__chevron', expanded && 'is-open')} size={16} />
      </button>
      <div className={uiClassName("stage-progress__bar")}>
        <span style={{ width: `${percent}%` }} />
      </div>
      <div className={uiClassName("stage-progress__steps")} aria-label="任务阶段">
        {stages.map((stage) =>
        <span
          key={stage.key}
          className={uiClassName('stage-progress__step', `is-${stage.status || 'pending'}`)}
      ...
      {expanded &&
      <div className={uiClassName("stage-progress__logs")}>
          {stages.map((stage) =>
        <div key={stage.key} className={uiClassName("stage-log")}>

...
import { FileUp, Plus, X } from 'lucide-react';import { uiClassName } from "../../utils/uiClassName";
...
const ImageUploadPreviewList = ({
  ...
  if (files.length === 0) {
    return (
      <label className={uiClassName("image-upload-preview image-upload-preview--empty")}>
        <FileUp size={18} />
  ...
  return (
    <div className={uiClassName("image-upload-preview")}>
      <div className={uiClassName("image-upload-preview__grid")} role="list">
        {previewItems.map(({ file, index, url, key }) =>
        <article className={uiClassName("image-upload-preview__card")} key={key} role="listitem">
            <div className={uiClassName("image-upload-preview__thumb")}>
              <img src={url} alt={`${itemLabel} ${index + 1}: ${file.name}`} />
              <button
              type="button"
              className={uiClassName("image-upload-preview__remove")}
        ...
        <label className={uiClassName("image-upload-preview__add")}>
          <Plus size={22} />
          <span>{addLabel}</span>
          {input}
        </label>

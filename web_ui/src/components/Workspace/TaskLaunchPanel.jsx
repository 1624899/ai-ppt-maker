...
import { getJobTitle } from '../../utils/jobPresentation';import { uiClassName } from "../../utils/uiClassName";
...
const TaskLaunchPanel = ({
  ...
  return (
    <motion.main
      className={uiClassName("workspace-panel task-launch-panel")}
      aria-label={title}
      ...
      <header className={uiClassName("workspace-panel__header task-launch-panel__head")}>
        <div className={uiClassName("task-launch-panel__title")}>
          <span className={uiClassName("eyebrow")}>任务 Brief</span>
          <h2>{title}</h2>
          <p>{description}</p>
        </div>
        <button className={uiClassName("btn btn-secondary")} type="button" onClick={onClose}>
          收起到顶栏
        </button>
      </header>

      <div className={uiClassName("task-launch-panel__body")}>
        <aside className={uiClassName("task-launch-brief")}>
          {!isFromCurrentJob &&
          <div className={uiClassName("task-launch-brief__icon")}>
          ...
          <div className={uiClassName("task-launch-insights")} aria-label="参数解读">
              <strong>参数解读</strong>
          ...
          <div className={uiClassName("task-launch-summary")} aria-label="当前任务参数摘要">
              <strong>当前任务参数</strong>
              <div className={uiClassName("task-launch-brief__chips")}>
                {summaryItems.map((item) => <em key={item}>{item}</em>)}
              </div>
            </div>
          }
          <div className={uiClassName("task-launch-brief__note")}>
            <Sparkles size={16} />
            <small>任务提交后会像抽屉一样收起，参数仍会随任务保留在顶栏入口里。</small>
          </div>
        </aside>

        <section className={uiClassName("task-launch-form-card")}>
          <CreationForm
            compact

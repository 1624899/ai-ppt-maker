...
const AgentChatPanel = ({
  ...
  const clearConversation = async () => {
    if (!currentJob?.job_id || pending || clearing) return;
    setClearing(true);
    setClearError('');
    try {
      const result = await clearAgentConversation(currentJob.job_id);
      setMessages([]);
      setPendingDraft(null);
      setInput('');
      onDraftInstructionChange('');
      onConversationCleared?.(result);
    } catch (err) {
      setClearError(err.message || '清空记录失败');
    } finally {
      setClearing(false);
    }
  };
